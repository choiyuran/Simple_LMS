...
- **기능**
    - 일부 페이징, QueryDSL 조건 검색, 공지사항 등록 및 수정, 강의 등록 및 수정/상세보기, 강의실 사용 시간 체크, 교수/학생 조회 및 수정, 학과 조회 및 등록, 학생 상태 변경, 장학금 조회
...
- 이메일 주소의 형식이 유효하지 않거나 이미 가입된 이메일일 경우 또는 비밀번호가 6자 미만일 경우에는 각 입력창 하단에 경고 문구가 나타납니다.
...
- 점수와 주관식 평가의 내용을 확인할 수 있습니다.
- A,B,C,D,E 로 A가 매우 그렇다, E가 매우 아니다 입니다.
...
- 등록금을 납부할 수 있습니다.
- 등록금 납부는 실제로 납부할 수 없기 때문에 더미 계좌번호와 납부했다는 alert만을 남깁니다.
- 사진의 해당 학생은 전액 장학금을 받았기 때문에 최종 납부금액이 0원인 모습을 알 수 있습니다.
...
- 학생은 휴학신청 및 복학 신청을 하고 교직원이 이를 승인해야 복학상태, 휴학상태로 전환됩니다.
...
- 강의 계획서를 등록을해야 강의 상세보기에서 학생들이 수업을 파악하고 수강신청을 할 수 있도록 정보를 제공할 수 있습니다.
...
- **BackEnd**
  ...
  **QueryDSL**
  ...
  ⌛ AWS
  | 문제 상황 | 인스턴스 경보 및 상태체크 에러문제 |
  | --- | --- |
  | 원인 | 원인은 모르겠으나 상태체크 과정에서 에러가 생기고 이 에러로 인해 접속이 불가능해지는 문제가 생겼음 |
  | 해결 | 인스턴스 중지 후 다시 시작 이 과정에서 퍼블릭 IP주소가 바뀌는 단점이 있으나 해결했음 IP주소에 맞춰서 도메인 IP주소 변경 및 포트포워딩을 다시 해줌으로써 복구 완료 |
